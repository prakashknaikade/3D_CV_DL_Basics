...
def compute_albedo(calibration_file, images_dir, output_dir):
    ...
    pbar = tqdm(cameras.items(), desc='Processing cameras')

    # Iterate over each camera
    for cam_id, cam_data in pbar:
        # Update the description with the current camera ID
        pbar.set_description(f'Computing albedo for camera {cam_id}')
        ...
        cam_pos_world = -R_cam.T @ t_cam  # X_world ​= R_cam_⊤ ​(X_camera ​− t_cam​) , Shape: (3, 1) 

        # Ray-plane intersection at y = 0.25m
        plane_y = 0.25
        ry = cam_rays_world[1, :]
        py = cam_pos_world[1, 0]
        t_vals = (plane_y - py) / ry # Compute t where the ray intersects the plane (Yplane = py ​+ t * ry​​)
        
        # Identify valid t values (intersection in front of the camera)
        valid_t = (t_vals > 0) & np.isfinite(t_vals) 
        t_vals[~valid_t] = np.nan # np.inf  # Ignore rays that do not intersect plane

        # Compute world coordinates of the intersection points
        world_coords = cam_pos_world + cam_rays_world * t_vals  # Shape: (3, N)

        # Identify valid world_coords (finite values across all components)
        valid_world_coords = np.isfinite(world_coords).all(axis=0)  # Shape: (N,)

        # Update valid_pixels to exclude pixels with invalid world coordinates
        valid_pixels = valid_world_coords  # Combine with existing mask

        # Now, only process valid pixels
        world_coords_valid = world_coords[:, valid_pixels]  # Shape: (3, num_valid_pixels)
        ...
        for idx, (light_id, light_data) in enumerate(lights.items()):
            ...
            light_dirs = light_pos_world - world_coords_valid  # Shape: (3, num_valid_pixels)
            ...
            L[valid_pixels, idx, :] = light_dirs_normalized.T  # Shape: (N, 3)
        ...
        # V = np.einsum('ijk,ik->ij', L[valid_pixels], N_normalized[valid_pixels])  # V_i = (L_i * N) Shape: (num_valid_pixels, num_lights) 
        ...
        # Prepare valid observations mask
        valid_obs = (V > 0)[:, :, np.newaxis] & (I_valid > 0) & (I_valid < INTENSITY_THRESHOLD)
        # Shape of valid_obs: (num_valid_pixels, num_lights, num_channels)

        # Count valid observations per pixel and channel
        num_valid_obs = np.sum(valid_obs, axis=1)  # Shape: (num_valid_pixels, num_channels)

        # Identify pixels with sufficient observations
        sufficient_obs = num_valid_obs >= 3  # Shape: (num_valid_pixels, num_channels)

        # Mask the invalid observations by setting them to zero
        V_masked = np.where(valid_obs, V[:, :, np.newaxis], 0)  # Shape: (num_valid_pixels, num_lights, num_channels)
        I_masked = np.where(valid_obs, I_valid, 0)              # Shape: (num_valid_pixels, num_lights, num_channels)

        # Compute numerator and denominator for least squares
        VI_sum = np.sum(V_masked * I_masked, axis=1)       # Shape: (num_valid_pixels, num_channels)
        V_squared_sum = np.sum(V_masked ** 2, axis=1)      # Shape: (num_valid_pixels, num_channels)

        # Avoid division by zero
        valid_denominator = V_squared_sum > 1e-6
        rho = np.zeros_like(VI_sum)
        rho[valid_denominator] = VI_sum[valid_denominator] / V_squared_sum[valid_denominator]

        # Set albedo to zero where insufficient observations
        rho[~sufficient_obs] = 0

        # Clip albedo values to [0, 1]
        rho_clipped = np.clip(rho, 0, 1)

        # Assign the computed albedo to the full albedo array
        albedo_valid = rho_clipped
...
if __name__ == '__main__':
    ...
    output_dir = 'output_threshold_90_vectorized_1'
